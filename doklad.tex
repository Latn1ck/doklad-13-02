...
Отсюда вытекает, что
\begin{equation*}
	e^{s\cdot R_n(A)}=\exp\left[s\cdot\mathbb{E}\max\limits_{1\leqslant j\leqslant N}\frac{1}{n}\left|\sum\limits_{i=1}^{n}\sigma_ia_i^{(j)}\right|\right]\leqslant\mathbb{E} \exp\left[s\cdot\max\limits_{1\leqslant j\leqslant N}\frac{1}{n}\left|\sum\limits_{i=1}^{n}\sigma_ia_i^{(j)}\right|\right]\leqslant
\end{equation*}
\begin{equation*}
...
\end{equation*}
\begin{equation*}
	s\cdot R_n(A)\leqslant \ln N\cdot\max\limits_{1\leqslant j\leqslant N}\frac{s^2\cdot\|a^{(j)}\|^2}{2n^2}
\end{equation*}
\begin{equation*}
	R_n(A)\leqslant\frac{s\ln N}{2n^2}\cdot\max\limits_{1\leqslant j\leqslant N}\|a^{(j)}\|^2
\end{equation*}
...
\end{equation*}
Величина $\mathbb{S}_{\mathcal{F}}(x_1^n)$ называется \textbf{коэффициентом разбиения Вапника-Червоненкиса}, а её логарифм называется \textbf{размерностью Вапника-Червоненкиса}.
...
\begin{equation*}
	\left|\left\{k:y_k\cdot(\langle w,x_k\rangle-b) \leqslant0\right\}\right|\to\min
\end{equation*}

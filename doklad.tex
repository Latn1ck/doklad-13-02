...
Так как $nL_n(g)\sim Bi(n,L(g))$, то следует изучать отклонения биномиальных случайных величин от их математического ожидания. Сформулируем задачу более общо: пусть

\noindent$X_1$, $\dots$, $X_n$ --- i. i. d. случайные величины со значениями в некотором множестве $\mathscr{X}$ и

\noindent $\mathcal{F}$ --- класс функций $f:\mathscr{X}\to[-1,1]$. Обозначим
\begin{equation*}
...
\begin{theorem}[неравенство ограниченных разностей]\label{thBoundedDifferences}
	...
	\begin{equation*}
		\mathbb{P}\left\{Z-\mathbb{E}Z>t\right\}\leqslant e^{-\frac{2t^2}{C}}, \quad C=\sum\limits_{i=1}^{n}c_i^2
	\end{equation*}
(\textbf{оно же неравенство МакДиармида}).
\end{theorem}
...
Последнее неравенство позволяет сконцентрироваться на математическом ожидании, которое можно оценить сверху с помощью симметризации. Обозначим $X_1'$, $\dots$, $X_n'$ --- <<призрачная>> выборка, состоящая из независимых случайных величин, которые также являеются независимыми с $X_i$, распределение которых совпадает с $X_i$.
\begin{equation*}
...
\end{equation*}
\begin{equation*}
\leqslant2\mathbb{E}\left[\sup\limits_{f\in\mathcal{F}}\frac{1}{n}\sum\limits_{i=1}^{n}\sigma_if(X_i)\right]
\end{equation*}

Пусть $A\subset\mathbb{R}^n$ --- ограниченное, $a=(a_1,\dots, a_n)\in A$. \textbf{Сложность Радемахера} по множеству $A$:
\begin{equation*}
R_n(A):=\mathbb{E}\left[\sup\limits_{a\in A}\frac{1}{n}\sum\limits_{i=1}^{n}\sigma_ia_i\right]
\end{equation*}
...
Пусть $A$, $B\subset\mathbb{R}^n$ --- ограниченные, $c\in\mathbb{R}$, $b\in\mathbb{R}^n$, тогда
\begin{enumerate}
...
\end{equation*}
\begin{equation*}
	\operatorname{conv}A=\left\{\sum\limits_{j=1}^{N}c_ja_j: N\in\mathbb{N}, c_j\geqslant0, \sum\limits_{j=1}^{N}c_j\leqslant1, a_j\in A\right\},
\end{equation*}
то $R_n(\operatorname{absconv}A)=R_n(\operatorname{conv} A)=R_n(A)$.
\end{enumerate}
\end{theorem}

\textbf{Доказательство свойства \eqref{eqProperty5}}.  Вытекает из неравенства Хёфдинга, которое утверждает, что если ограниченная случайная величина $\xi$ имеет нулевое математическое ожидание и принимает значения на $[\alpha,\beta]$, то для любого $s>0$
\begin{equation*}
...
Рассмотрим случай, когда класс $\mathcal{F}$ состоит только из функций-индикаторов. Тогда для любого набора точек $x_1^n=(x_1,\dots,x_n)$ множество $\mathcal{F}(x_1^n)$ конечно, его мощность $\mathbb{S}_{\mathcal{F}}(x_1^n)\leqslant2^n$ и по свойству $\eqref{eqProperty5}$
...
VC-размерность класса $\mathcal{F}$ --- это наибольшее число $h$ такое, что найдётся подмножество мощности $h$ такое, что функции из $\mathcal{F}$ смогут разбить его на два класса всеми возможными способами.
...
\end{example}
\begin{equation*}
	R_2(A)=\frac{1}{2}\cdot\mathbb{E}\left[\sigma_1 a+\sigma_2b\right]=0
\end{equation*}
\begin{example}
	\begin{equation*}
		A=\{(a_1,a_2), (b_1,b_2)\}
...
\end{example}
\begin{equation*}
	R_2(A)=\frac{1}{2}\cdot\mathbb{E}\left[\max\{a_1\sigma_1+a_2\sigma_2,b_1\sigma_1+b_2\sigma_2\}\right]=
\end{equation*}
\begin{equation*}
	=\frac{1}{2}\cdot\left[\frac{1}{4}\cdot\max\{a_1+a_2,b_1+b_2\}+\frac{1}{4}\cdot\max\{a_1-a_2,b_1-b_2\}+\frac{1}{4}\cdot\max\{-a_1+a_2,-b_1+b_2\}+\right.
\end{equation*}
\begin{equation*}
	\left.+\frac{1}{4}\cdot\max\{-a_1-a_2,-b_1-b_2\}\right]=\frac{1}{8}\cdot\left[\max\{a_1+a_2,b_1+b_2\}-\min\{a_1+a_2,b_1+b_2\}+\right.
\end{equation*}
\begin{equation*}
	\left.+\max\{a_1-a_2,b_1-b_2\}-\min\{a_1-a_2,b_1-b_2\}\right]
\end{equation*}
Распадается на 4 случая, в итоге
\begin{equation*}
	R_2(A)=\frac{1}{4}\cdot\max\left\{|b_1-a_1|,|b_2-a_2|\right\}
\end{equation*}
Так как $\operatorname{conv}A=\left[(a_1,a_2), (b_1,b_2)\right]$, то нашли сложность Радемахера отрезка на плоскости.
\begin{example}
	\begin{equation*}
...
\end{example}
\begin{equation*}
	R_2(A)=\frac{1}{2}\cdot\mathbb{E}\left[\sup\limits_{x^2+y^2\leqslant 1}(\sigma_1x+\sigma_2y)\right]=\frac{1}{2}\cdot\left[\frac{1}{4}\cdot\sup\limits_{x^2+y^2\leqslant 1}(x+y)+\frac{1}{4}\cdot\sup\limits_{x^2+y^2\leqslant 1}(x-y)+\right.
\end{equation*}
\begin{equation*}
	\left.+\frac{1}{4}\cdot\sup\limits_{x^2+y^2\leqslant 1}(-x+y)+\frac{1}{4}\cdot\sup\limits_{x^2+y^2\leqslant 1}(-x-y)\right]=\frac{\sqrt{2}+\sqrt{2}+\sqrt{2}+\sqrt{2}}{8}=\frac{1}{\sqrt{2}}
\end{equation*}
В общем случае
\begin{equation*}
	R_n(B[0,1])=\dfrac{1}{\sqrt{n}}, \quad R_n(B[x,r])=\dfrac{r}{\sqrt{n}}
\end{equation*}
...
\textbf{Доказательство}. Обозначим $\mathcal{H}$ --- класс функций $\mathcal{X}\times\{-1,1\}\to\mathbb{R}$ вида $-f(x)y$, где $f\in\mathcal{F}$, тогда учитывая свойство $\eqref{eqProperty6}$ теоремы $\ref{thProperties}$, имеем
\begin{equation*}
...
В этой случае, учитывая свойство $\eqref{eqProperty7}$ и неравенство $\eqref{eqRademacherVCInequality}$, имеем
\begin{equation*}
...
Подводя итог, мы получили, что если $\mathcal{F}_\lambda$ имеет указанный выше вид, то для любой выбранной функции $f_n$
из $\mathcal{F}_\lambda$ на основе данных, вероятность ошибки соответствующего классификатора удовлетворяет с вероятностью $\geqslant1-\delta$ неравенству
\begin{equation*}
...
Алгоритмы, работающие с ядрами, обычно выполняют минимизацию функционала штрафа на шаре соответствующего воспроизводящего ядра гильбертова пространства вида
\begin{equation*}
...
\begin{theorem}[Избыточный риск выпуклого минимизатора риска]
	...
	\noindent$f_n$ выбран из $\mathcal{F}_\lambda$, где $\mathcal{F}_\lambda$ определён в $\eqref{eqWeightedClasses}$, путём минимизации эмпирического функционала штрафа $A_n(f)$ с использованием экспоненциальной либо логистической функции штрафа. Тогда для любого $\delta\in(0,1)$ с вероятностью $\geqslant1-\delta$ выполняется неравенство
	\begin{equation*}
